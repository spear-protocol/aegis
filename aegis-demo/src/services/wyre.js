...
class WyreWrapper {
  async configure () {
    let deviceToken = window.localStorage.getItem(DEVICE_TOKEN__KEY)
    if (!deviceToken) {
      const array = new Uint8Array(25)
      window.crypto.getRandomValues(array)
      deviceToken = Array.prototype.map.call(array, x => ('00' + x.toString(16)).slice(-2)).join('')
      window.localStorage.setItem(DEVICE_TOKEN__KEY, deviceToken)
    }
  }
  ...
  displayWidget (
    ...
    const deviceToken = window.localStorage.getItem(DEVICE_TOKEN__KEY)
